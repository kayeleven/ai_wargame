...
def test_login_user_and_two_scenario_workflows(admin_server, javascript):
    with sync_playwright() as playwright:
        ...
        page.get_by_text("Assign game role", exact=True).click()
        role_form = page.locator('form[action="/admin/roles"]')
        role_form.get_by_label("User", exact=True).select_option(label="planner")
        role_form.get_by_label("Game", exact=True).select_option("harbor-relief")
        role_form.get_by_label("Role", exact=True).select_option("adjudicator")
        role_form.get_by_role("button", name="Assign role").click()
        page.get_by_text("Assign team membership", exact=True).click()
        member_form = page.locator('form[action="/admin/memberships"]')
        member_form.get_by_label("User", exact=True).select_option(label="planner")
        member_form.get_by_label("Game identifier").fill("harbor-relief")
        member_form.get_by_label("Team identifier").fill("team-0")
        member_form.get_by_label("Authority").select_option("submitter")
        member_form.get_by_role("button", name="Assign membership").click()
        page.get_by_role("link", name="Harbor Relief", exact=True).click()
        change_form = page.locator('form[action$="/change"]')
        change_form.get_by_label("Authority").select_option("member")
        change_form.get_by_role("button", name="Change authority").click()
        expect(page.get_by_text("team-0: No active submitter is assigned")).to_be_visible()
        page.get_by_role("button", name="Remove membership").click()
        page.get_by_role("button", name="Remove role", exact=True).click()
        expect(page.get_by_role("heading", name="Governing configuration")).to_be_visible()
        expect(page.get_by_role("heading", name="Configuration history")).to_be_visible()
        revision_form = page.locator('form[action="/admin/games/harbor-relief/revisions"]')
        revision_form.get_by_label("Configuration", exact=True).fill("{")
        revision_form.get_by_role("button", name="Create immutable revision").click()
        expect(page.get_by_role("heading", name="Review your input")).to_be_visible()
        page.goto(admin_server + "/admin")
        page.get_by_role("button", name="Mark read", exact=True).click()
        expect(page.get_by_role("button", name="Mark read", exact=True)).to_have_count(0)
